 ...
 * ----------------------------------------------------------------------------------
 * License MIT - Serge Koriakin - Jule 2012-2018
 * ----------------------------------------------------------------------------------
 */
...
//debug($name,$par);
    ...
    static public function set_option($name = '', $value = null, $transport = '')
    {
        ...
        if (!empty($transport) && is_string($transport)){
            if(array_key_exists($transport, self::$transports)) {
                ...
                $x = explode('|', $transport . '|');
                $n = __NAMESPACE__ . '\engine_options_' . $x[0];
                if (is_callable(array($n, 'init')))
                    self::$transports[$transport] =
                        call_user_func(array($n, 'init'), $transport, $x[1]);
                else
                    self::$transports[$transport] = $n;
                //ENGINE::debug($transport, self::$transports[$transport]);
                $transport = self::$transports[$transport];
            }
        }
    ...
    static public function read_options($transport = '')
    {
        if (!empty($transport)
            ...
            $x = explode('|', $transport . '|');
            $y = explode('~', $x[0] . '~');
            $n = __NAMESPACE__ . '\engine_options_' . $y[0];
            if (is_callable(array($n, 'init')))
                self::$transports[$transport] =
                    call_user_func(array($n, 'init'), $y[1], $x[1]);
            else
                self::$transports[$transport] = $n;
        }
    }
